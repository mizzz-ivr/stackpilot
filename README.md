...
保存前プレビューでは、自動判定されない項目名を次のカテゴリへ一時的に追加できます。

- URL query名
- Request / Response header名
- JSON / `application/x-www-form-urlencoded`のフィールド名

項目名はカンマまたは改行で区切ります。大文字小文字、`-`、`_`、camelCaseの差異を正規化して照合します。各カテゴリは最大20件、各項目名は最大64文字です。空値、制御文字、正規化後の重複は受け付けません。

追加ルールを入力した後は`追加ルールで再プレビュー`を押し、成果物・SHA-256・マスキング件数が更新されたことを確認してから保存します。入力内容が現在のプレビューへ未反映の間は保存できません。
...
自動マスキング済みの値を解除することはできません。URL path、正規表現、値ベース置換、JSON / form以外のbody形式は追加ルールの対象外です。
...
rendererからmain processへ渡すのは、プレビュー生成時のWorkspace ID・形式・フィルター・追加対象の項目名配列と、保存時のpreview IDだけです。ログ本文、raw値、成果物本文、保存先パスをrendererから指定することはできません。保存先はElectronの保存ダイアログでユーザーが選択します。
