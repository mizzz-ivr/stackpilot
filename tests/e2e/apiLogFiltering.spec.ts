...
const comparisonPath = '/health?region=jp';
...
test('API通信を検索・絞り込み・ピン留めできる', async ({ appWindow }) => {
  ...
  await expect(appWindow.getByText(comparisonPath, { exact: true })).toBeVisible();
  await expect(appWindow.getByText('2 / 2件')).toBeVisible();
  ...
  await expect(appWindow.getByText('0 / 2件')).toBeVisible();

  await appWindow.getByRole('button', { name: '検索キーワードを消去', exact: true }).click();
  await expect(appWindow.getByText(fixturePath, { exact: true })).toBeVisible();

  const targetRow = appWindow.getByText(fixturePath, { exact: true }).locator('..').locator('..');
  const pinButton = targetRow.getByRole('button', { name: '通信をピン留め', exact: true });
  await pinButton.click();
  await expect(targetRow.getByRole('button', { name: '通信のピン留めを解除', exact: true })).toBeVisible();
  ...
  await expect(appWindow.getByText(comparisonPath, { exact: true })).toBeVisible();
  await expect(appWindow.getByText(fixturePath, { exact: true })).toBeHidden();
  await appWindow.getByLabel('statusで絞り込み', { exact: true }).selectOption('success');
  await expect(appWindow.getByText(fixturePath, { exact: true })).toBeVisible();
  await expect(appWindow.getByText(comparisonPath, { exact: true })).toBeHidden();
});
